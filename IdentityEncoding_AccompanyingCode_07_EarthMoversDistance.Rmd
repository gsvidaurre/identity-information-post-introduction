---
title: <center style="font-size:30px;font-style:normal;color:black;">Accompanying Code 07:</center>
subtitle: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Earth Mover's Distance</center>
 &nbsp;
author: |
...
We used Earth Mover's Distance<a href='#References'><sup>[1]</sup></a> to quantify the strength of acoustic convergence at each social scale for native and introduced range monk parakeet calls. This analysis is based on thinking about distributions of similarity values within and among categories at each social scale as two features of convergence.

We calculated this distance metric between distributions of similarity values within versus among categories at a given social scale (e.g. within versus among individuals at the individual scale). To do this, we use a histogram-based approach to create signatures that are compared between the distributions. Since we did not have **a priori** expectations for an optimal number of bins to use, we iterate over different numbers of bins to calculate Earth Mover's Distance. For each number of total bins, we also randomly sample an equivalent number of similarity values from each distribution over various iterations.
...
DEPCRECATE THIS SAMPLE SIZE SECTION....this was just for Mantel tests...
We used 5 native range and 5 introduced range individuals for Mantel tests at the individual scale. Although these native range individuals were sampled at a single site (site 1145 sampled in 2017), and the introduced range individuals sampled at 3 sites (city of Austin in 2019), a previous validation analysis found that these individuals represented similar patterns of acoustic variation (CITE). As geographic distance increases, individuals overlap more in acoustic space (CITE). Aside from the introduced range individuals recorded in Austin, most introduced range individuals were recorded over very large geographic distances. As such, we used native range birds from site 1145 and introduced range birds from Austin 2019 for the fairest comparison possible of individual signatures in our dataset. 
...
invisible(pblapply(1:length(ranges), function(k){
  ...
  lapply(1:iteratns, function(p){
    ...
    emd_res_df <- resampled_earthMoversDistance(sim_df = tmp_sim_df, social_scale = "Individual", range = rng, dataset = NA, similarity_method = "SPCC", city_year = NA, distr_types = distr_types, total_bins = total_bins, bounds = bounds)
  ...
  lapply(1:length(sim_meth), function(s){
    ...
    lapply(1:length(data_subset), function(z){
      ...
      lapply(1:iteratns, function(p){
        ...
        # Do the Earth Mover's Distance calculations over bin numbers and resampling
        emd_res_df <- resampled_earthMoversDistance(sim_df = tmp_sim_df, social_scale = "Site", range = rng, dataset = data_subset[z], similarity_method = sim_meth[s], city_year = NA, distr_types = distr_types, total_bins = total_bins, bounds = bounds)
...
Read in the Earth Mover's distance results.
```{r echo = TRUE, eval = TRUE}
...
# emd_res %>%
#   distinct(range, social_scale, similarity_method, dataset, iter) %>% 
#   View()
...
Summarize the Earth Mover's Distance results across social scales.
```{r echo = TRUE, eval = TRUE}

glimpse(emd_res)

# First check how consistent the EMD results are across resampling iterations as well as bins. More consistency in the native range compared to the introduced range, and this may well be due to geography spread in the introduced range individual scale sampling (e.g. more likely that individuals far apart will develop similar individual signatures)
...
# If results are consistent across bins within resampling iterations, then select one bin number and summarise across iterations to get means and 95% CIs across social scales, ranges, datasets, similarity methods. Make plots for the supplementary materials
...
# Use an intermediate bin number since results are consistent across bins (within sampling iterations). I decided to go with 16 bins for the results that will go into a table in the main ms
...
ggplot(data = sum_stats_df,
  ...
  geom_segment(aes(x = 0.45, xend = 1.55, y = 0.19, yend = 0.19), color = "black", linetype = "solid", size = 0.15) +
  # Annotation line above the site scale results
  geom_segment(aes(x = 1.75, xend = 3.25, y = 0.19, yend = 0.19), color = "black", linetype = "solid", size = 0.15) +
...
Read in the 3 site scale datasets for temporal comparisons.
```{r echo = TRUE, eval = TRUE}
...
Here I randomly sampled 5 calls per site with more than 5 calls but I randomly sampled 2 sites per city-year given that we generally did not sample 5 different sites in each city-year.
```{r echo = TRUE, eval = FALSE}
    ...
    lapply(1:length(sim_meth), function(s){
      ...
      lapply(1:length(data_subset), function(z){
        ...
        lapply(1:iteratns, function(p){
          ...
          # 25 among values and 20 within...for the random sampling below in the Earth Mover's Distance calculations, looks good
          # tmp_sim_df %>%
          #   group_by(type) %>%
          #   dplyr::summarise(
          #     n = n()
          ...
          # Do the Earth Mover's Distance calculations over bin numbers and resampling
          emd_res_df <- resampled_earthMoversDistance(sim_df = tmp_sim_df, social_scale = "Site", range = unique(tmp_df$range), dataset = data_subset[z], similarity_method = sim_meth[s], city_year = unique(tmp_sim_df$city_year), distr_types = distr_types, total_bins = total_bins, bounds = bounds)
...
Summarize the Earth Mover's distance results for this temporal analysis.
```{r echo = TRUE, eval = TRUE}
...
# If results are consistent across bins within resampling iterations, then select one bin number and summarise across iterations to get means and 95% CIs across datasets and similarity methods for each city-year. Make plots for the supplementary materials
...
ggplot() +
  geom_line(data = sum_stats_temp_df %>%
            ...
            aes(x = year, y = mean, linetype = similarity_method, group = similarity_method), color = clrs, size = 0.75) +
...
ggplot() +
  ...
  theme(
    ...
    strip.text = element_blank(),
    axis.text = element_text(size = 12)
    # Removing y-axis labels and ticks because these are the same as the plot for Austin
...
emd_res %>% 
  ...
  dplyr::summarise(
    ...
    l_CI = round(Rmisc::CI(EMD, ci = 0.95)[["lower"]], 3),
    u_CI = round(Rmisc::CI(EMD, ci = 0.95)[["upper"]], 3)
    # Checking that 95% CIs are just as tight around the mean but using a different package
    # mean = round(confintr::ci_mean(EMD)[["estimate"]], 3),
    # l_CI = round(confintr::ci_mean(EMD)[["interval"]][1], 3),
    # u_CI = round(confintr::ci_mean(EMD)[["interval"]][2], 3)
) %>% 
  # Arrange to match ordering of Table 1 in the main text
  dplyr::arrange(.,
    similarity_method, city, year, dataset
...
# References
    ...
    1. Rubner, Y., Tomasi, C., & Guibas, L. J. 2000. The Earth Mover’s Distance as a metric for image retrieval. International Journal of Computer Vision, 40(2), 99.
