---
title: <center style="font-size:30px;font-style:normal;color:black;">Accompanying Code 10:</center>
subtitle: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Mantel Tests</center>
 &nbsp;
author: |
...
Mantel tests were conducted per range to evaluate the strength and significance of acoustic convergence across social scales. For the site scale, Mantel tests were conducted over similarity methods: spectrographic cross-correlation (SPCC) and random forests, as well as over the 3 datasets used to address potential repeated sampling of individuals: no filtering (full dataset), filtering by Gaussian mixture models, and filtering by visual classification. See the supplement of the associated publication for more information about these analyses.
...
X <- c("tidyverse", "pbapply", "plyr", "dplyr", "data.table", "parallel", "vegan", "knitr", "rlist")
...
This analysis addresses the first half of the hierachical mapping analysis (individual scale, site scale needs filtering and analyses run in triplicate for the potential repeated sampling issue). Mantel tests were performed with all native and all introduced individuals, then with individuals recorded over restricted geographic areas (a single site-year for the native range, a single city-year for the introduced range). A partial Mantel test was conducted for the introduced range birds to control for site identity, as the introduced range birds were recorded at 3 different sites. We used only SPCC similarity for Mantel tests at the individual scale, as random forests models had been trained on individual scale calls.
...
# Filter the random forests similarity matrix by the calls used for prediction
# 
...
Individual scale p-values are significant for both ranges at the adjusted alpha. Some interesting site scale patterns. Most notably, the higher magnitude Mantel test statistic at the site scale for the introduced range with the full unfiltered call dataset. Mantel test statistics are lower for the introduced range for the two filtered datasets, regardless of the similarity method used.

### Validation of retention of UM4 calls for native range site scale

Mantel test for the native range site scale dataset with the UM4 calls. Here, I compared Mantel test statistics, not significance. I also ran a Mantel test with just SPCC similarity here.
```{r echo = TRUE, eval = TRUE}

# Read in the full extended selection table
nat_int_est <- readRDS(file.path(path, "monk_parakeet_contactCalls_rangeComparison_extSelTable.RDS"))
# glimpse(nat_int_est)
...
# Note that there are 602 calls here (610 in the extended selection table prior to dropping calls earlier in the script) because I added back one call each for a few more repeatedly sampled individuals in this work
...
Mantel test statistics were very similar with and without the UM4 calls for the native range out to 4 significant digits, and are the same when rounded to 2 digits (as reported in previous work<a href='#References'><sup>[1]</sup></a>). Therefore, the inclusion of these UM4 calls at the site scale in previous work did not influence the findings previously reported.

# Mantel tests over time in the introduced range

Initialize the 3 site scale datasets for temporal comparisons.
```{r echo = TRUE, eval = FALSE}
