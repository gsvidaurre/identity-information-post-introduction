---
title: <center style="font-size:30px;font-style:normal;color:black;">Accompanying Code 08:</center>
subtitle: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Benchmarking Species Comparison</center>
 &nbsp;
author: |
...
We compared hierarchical mapping patterns between monk parakeets and yellow-naped amazons using a customized bootstrapping routine and SPCC similarity. See the methods and appendix of the associated publication for more information about these analyses.
...
For yellow-naped amazon calls, I obtained similarity within and among sites within the Northern dialect, as this dialect has differences among sites and has been shown to be relatively stable over time<a href='#References'><sup>[1]</sup></a><a href='#References'><sup>[2]</sup></a>. I also obtained similarity within and among sites between the Northern and Southern dialects, two previously described regional dialects in yellow-naped amazons. Since multiple calls were sampled per individual, I randomly sampled just one call per repeatedly sampled bird.

Perform SPCC for the Northern dialect yellow-naped amazon calls. Note that these YNA calls are at a lower sampling rate than current monk calls. For another comparison of acoustic variation between species and similarity methodes, see previous work<a href='#References'><sup>[3]</sup></a>. Using the same window length for yellow-naped amazon and monk parakeet calls in SPCC.
...
yna_sels_df <- rbindlist(pblapply(1:length(wavs), function(w){
  ...
  return(data.frame(sound.files = wavs[w], selec = 1, start = 0, end = duration(tmp), regional_dialect = strsplit(wavs[w], split = "_")[[1]][4], site = strsplit(wavs[w], split = "_")[[1]][5], bird_ID = paste(strsplit(wavs[w], split = "_")[[1]][5], gsub(".([0-9]+).wav", "", strsplit(wavs[w], split = "_")[[1]][6]), sep = "-"), sampling_rate = tmp@samp.rate))
}))

glimpse(yna_sels_df)

# all .wav files at 22.050 kHz sampling rate
...
Downsampling the EST to 22.05 kHz to match yellow-naped amazon sampling rates. Downsample all 1582 calls per range.
...
Reran SPCC with the downsampled monk parakeet calls.
```{r echo = TRUE, eval = FALSE}
...
I performed a bootstrapping analysis to compare SPCC similarity within and among sites for monk parakeets to yellow-naped amazons (YNA), a species with well-documented convergence at the site scale, and regional dialects<a href='#References'><sup>[1]</sup></a>. For the monk parakeet data, I used all 3 site scale datasets, and performed bootstrapping for a comparison between ranges, as well as a comparison over time in the introduced range.

I ran SPCC for yellow-naped amazons below. Monk parakeet site scale calls were downsampled to 22.05kHz to match the sampling rate for yellow-naped amazons, and I performed SPCC on the downsampled monk parakeet calls. 

### Yellow-naped amazon SPCC similarity across 3 social scales
...
Make an acoustic space plot to be sure the SPCC values recapitulate the patterns we expect for amazons.
```{r amazon acoustic space validation, eval = FALSE}
...
```{r checking amazon values}

x11()

paircomp_yna_df %>%
  ggplot() +
  geom_density(aes(x = value), size = 0.5) +
  ggtitle("") +
...
Get pairwise comparisons at the individual, site, and regional dialect social scales for yellow-naped amazon calls. I'm using the customized function I designed to extract similarity values (no values along the diagonal).
...
indiv_comparisons_df <- extract_simValues(sim_mat = xc_mat_yna, calls = indiv_calls, species = "yellow-naped amazon", social_scale = "Individual", dataset = NA, similarity_method = "SPCC", city_year = NA, metadata_df = yna_sels_df)
...
# Extract the similarity values for the pairwise comparisons of interest at this social scale
site_comparisons_df <- extract_simValues(sim_mat = xc_mat_yna, calls = site_calls, species = "yellow-naped amazon", social_scale = "Site", dataset = NA, similarity_method = "SPCC", city_year = NA, metadata_df = yna_sels_df)
...
# Extract the similarity values for the pairwise comparisons of interest at this social scale
dialect_comparisons_df <- extract_simValues(sim_mat = xc_mat_yna, calls = dialect_calls, species = "yellow-naped amazon", social_scale = "Regional Dialect", dataset = NA, similarity_method = "SPCC", city_year = NA, metadata_df = yna_sels_df)
...
Combine these pairwise comparisons into a single data frame for the bootstrapping analysis that follows. These similarity values represent unique pairwise comparisons among calls. 
```{r}
...
```{r bootstrapping within and among groups across amazon social scales}
...
iter <- 200
n <- 5 # the number of comparisons that will be sampled with replacement

# i <- 3
# x <- 2
# z <- 1 # testing
yna_bs_sim_ratio_df <- rbindlist(pblapply(1:length(social_scales), function(i){
  ...
  bs_df <- rbindlist(lapply(1:length(ids), function(x){
    ...
    bs_df2 <- rbindlist(lapply(1:iter, function(z){
      ...
      # Get pairwise comparisons within the given group     
      w_df <- tmp_df %>%
        dplyr::filter(!!rlang::sym(id_col_nm) == id) %>%
        dplyr::filter(!!rlang::sym(comp_col_nm) == "same") %>% 
        as_tibble()
      
      # Get pairwise comparisons among groups 
      # Given the way that I wrote the customized function extract_sim_values(), there is only 1 set of pairwise "among" comparisons (since I dropped duplicated pairwise comparisons in that function) 
      a_df <- tmp_df %>%
        dplyr::filter(!!rlang::sym(comp_col_nm) == "different") %>% 
        as_tibble()
...
Get similarity values for pairwise call comparisons at the individual and site scales for native and introduced range monk parakeets. At the site scale, I also need to repeat this process across datasets. I'm using the customized function I designed to extract similarity values (no values along the diagonal).

For comparison among native and introduced range monk parakeets to yellow-naped amazons, I used the same 5 native range and 5 introduced range individuals for Mantel tests at the individual scale, sampled at a single site or 3 nearby sites (that I considered a single site for these analyses), respectively. I chose native range sites from the department of Colonia (native range, recorded in 2017) or the city of Austin (introduced range recorded in 2019), representing a geographic area similar to that of the Northern dialect region for yellow-naped amazons.

Individual scale comparison: same or different individual, same site, same range
```{monk parakeets, individual scale}
...
inv_sites
length(inv_sites) # 6 sites

site_calls <- nat_int_est %>%
  as_tibble() %>% 
  dplyr::filter(social_scale == "Site") %>% 
  dplyr::filter(site_year %in% c(nat_sites, inv_sites)) %>% 
  pull(sound.files)
...
Extract SPCC values for monk parakeets over social scales, ranges, and for the site social scale, datasets as well. These SPCC values are saved in a .csv to facilitate making visuals later on.
```{r get SPCC values within and among groups across monk parakeet social scales}
...
# i <- 1
# z <- 1 # testing
mnk_spcc_vals_df <- rbindlist(pblapply(1:length(social_scales), function(x){
...
In order to compare results for native and introduced range monk parakeets to yellow-naped amazons, I performed bootstrapping of SPCC similarity values in a way as similar as possible to the yellow-naped amazon bootstrapping routine at the site scale. Over 1000 bootstrapping iterations, I randomly sampled 5 sites with replacement. For each of these 5 randomly sampled sites in each iteration, I randomly sampled 4 calls without replacement, or took all calls if the given site-year had 4 calls or less for the given dataset. This mimicked the sampling of one call per 2-4 unique individuals without replacement for yellow-naped amazons, as we considered each call in the monk parakeet site scale dataset a "unique" individual. This bootstrapping was performed over ranges and datasets, and the ratio of mean site scale similarity within compared to among sites was obtained per iteration.
...
For monk parakeets in each range:
  ...
  Site Scale: Sample 5 same and 5 different comparisons among sites after randomly sampling sites as described above
  
```{r monk parakeet bootstrapping}
...
# Checking. There are NAs, as expected
any(is.na(mnk_bs_sim_ratio_df$bootstrapped_sim_ratios))
length(which(is.na(mnk_bs_sim_ratio_df$bootstrapped_sim_ratios)))

# All the NAs (200 only) that occurred were for the introduced range site MANO 2019 for the visual classification datasets, meaning that for this site there were no bootstrapped similarity ratios because 5 similarity values within the site weren't available in this dataset
mnk_bs_sim_ratio_df %>% 
  dplyr::filter(is.na(bootstrapped_sim_ratios)) %>%
  group_by(range, dataset, social_scale, target_group) %>%
  dplyr::summarise(n = n()) 

# Looks good, 1000 similarity ratios for each category except site MANO in the visual classification dataset
mnk_bs_sim_ratio_df %>% 
...
yna_bs_sim_ratio_df %>% 
  ...
  theme(
    ...
    strip.background.y = element_rect(size = 0.25, color = "black", fill = "grey93"),
    strip.background.x = element_rect(size = 0.25, color = "black", fill = "grey93"),
...
# fills <- c(alpha("firebrick3", 0.45), alpha("navy", 0.35))
# cols <- c("firebrick3", "navy")
...
gg_df %>%
  ...
  geom_density(aes(x = similarity_value, fill = type, color = type), size = 0.25, bw = "nrd0") +
  # Add lines for medians per distribution
  geom_segment(data = medians_df, aes(x = median_val, xend = median_val, y = 0, yend = y_lim, color = type), linetype = "dashed", size = 0.50) +
  # Add a bar above the distributions showing the separation between the two
  geom_segment(data = sep_bars_df, aes(x = Same, xend = Different, y = y_lim, yend = y_lim), color = "black", linetype = "solid", size = 2.50) +
...
yna_sim_rat <- yna_bs_sim_ratio_df %>%
  ...
  # Get the overall mean and CI of the bootstrapped similarity ratios
  dplyr::summarise(
    mean_sim_ratio = mean(bootstrapped_sim_ratios),
    sd_vals = sd(bootstrapped_sim_ratios)
    # upper_CI = Rmisc::CI(bootstrapped_sim_ratios, ci = 0.95)[1],
    # lower_CI = Rmisc::CI(bootstrapped_sim_ratios, ci = 0.95)[3]
  ) %>%
  ungroup() %>%
  dplyr::mutate(
    dataset = "Full"
...
# Add a color gradient to the plot background
# g <- rasterGrob(gray.colors(12)[7], width = unit(1, "npc"), height = unit(1, "npc"), interpolate = FALSE)

# grid.draw(g1) # checking, looks good
...
gg_bs_df %>%
  ...
  facet_grid(cols = vars(range_spp), scales = "fixed", drop = TRUE) +
    # facet_wrap(vars(range_spp), scales = "fixed", drop = TRUE) +
  # Add the gradient background
  # Strong convergence at 1 and onwards
  # annotation_custom(g, xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf) +
  # geom_errorbar(aes(x = social_scale, ymin = mean_sim_ratio - sd_vals, ymax = mean_sim_ratio + sd_vals), color = "black", size = 0.25, width = 0.001) +
...
gg_bs_df %>%
  ...
  facet_grid(cols = vars(range_spp), scales = "fixed", drop = TRUE) +
  # Add the gradient background
  # Stronger convergence 1 and onwards
  # annotation_custom(g, xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf) +
  # geom_errorbar(aes(x = social_scale, ymin = mean_sim_ratio - sd_vals, ymax = mean_sim_ratio + sd_vals), color = "black", size = 0.25, width = 0.001) +
  geom_point(aes(x = social_scale, y = mean_sim_ratio, color = strength, fill = strength), shape = 21, size = 2.75) +
...
# References
    ...
    1. Wright, T. F. (1996). Regional dialects in the contact call of a parrot. Proceedings of the Royal Society of London, B, 263, 867–872. https://doi.org/10.1098/rspb.1996.0128
    
    2. Wright, T. F., Dahlin, C. R., & Salinas-Melgoza, A. (2008). Stability and change in vocal dialects of the yellow-naped amazon. Animal Behavior, 76(3), 1017–1027. https://doi.org/10.1016/j.anbehav.2008.03.025
    
    3. Smith-Vidaurre, G., Araya-Salas, M., and T.F. Wright. 2020. Individual signatures outweigh social group identity in contact calls of a communally nesting parrot. Behavioral Ecology 31(2), 448-458. https://doi.org/10.1093/beheco/arz202
