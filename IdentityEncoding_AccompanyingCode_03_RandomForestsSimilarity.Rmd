---
title: <center style="font-size:30px;font-style:normal;color:black;">Accompanying Code 03:</center>
subtitle: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Random Forests Similarity </center>
 &nbsp;
author: |
...
# Doublecheck whether the e1071 and MLmetrics packages are necessary
X <- c("warbleR", "ggplot2", "caret", "MASS", "dtw", "pbapply", "graphics",  "dplyr", "data.table", "parallel", "grid", "gtable", "corrplot", "MLmetrics", "e1071", "mclust", "tidyverse", "Boruta", "edarf", "viridis", "knitr", "pracma", "ranger")
...
SPCC similarity based on spectrograms and Mel-frequency cepstral coefficients was performed for all calls in an earlier script. See HierarchicalMapping_AdditionalMaterials03_SPCC_VisualClassificationPrep.Rmd. 

# Random forests similarity 

## Training plan

Known repeatedly sampled individuals (individual scale calls) were selected for model training. The same repeatedly sampled native range individuals were be used as in previously published results<a href='#References'><sup>[1]</sup></a>. These native range individuals were all from the same site. We did not sample sufficent individuals repeatedly at the same site in the introduced range, so for this range we used known repeatedly sampled individuals with the most calls. We excluded ASCA_2019 as this individual had no social group members (a lone parakeet in El Paso, TX). In sum, we used 4 individuals per range with the highest numbers of calls for model training, and introduced range birds used for training were sampled from different sites and years. 
```{r chunk4}
...
# sp_ts <- sp.en.ts(nat_int_est, wl = 378, length.out = 100, wn = "hanning", ovlp = 90, bp = c(0.5, 9), threshold = 15, img = FALSE, parallel = cores, img.suffix = "spec_ent", pb = TRUE, clip.edges = TRUE, leglab = "spec_ent", sp.en.range = c(0.5, 9), path = path, flim = c(0, 10))
# str(sp_ts)
...
# Returns distances by default
# The first run used window.type = "none", which yielded zeroes off the diagonal
# When using window.type = "sakoechiba" and window.size = 10, there were still zeroes off the diagonal for calls that are not duplicates (e.g 2017_05_21_PLVE_1005_10.WAV and 2017_05_21_PLVE_1005_11.WAV)
 # I retained results from window.type = "none" and adjusted zeroes off the diagonal (see)
...
MDS features were extracted from similarity matrices. We did not find an "optimal" number of dimensions as in previous work. Instead, I used the largest number of MDS dimensions that I could perform on my computer. 
...
(sup_rf_df %>%
  ...
  filter(social_scale == "Individual"))) * 100
...
A random forests model was trained using native and introduced individuals and all predictors. Model training and tuning were performed with 5 iterations of repeated 10-fold cross-validation. This was a multiclass model (e.g. 8 individuals = 8 class labels). I used the `ranger` package random forests implementation.
...
# ggsave(file.path(gpath, "SupplementaryFigure11_RFModel1_VarImp.tiff"), units = "in", width = 8, height = 6, dpi = 300)
  
```

# Random Forests Model 2

Could classification performance be improved by dropping less important features? I proceeded with automatic feature selection to drop less important variables in order to build a second random forests model. I did not pursue manual feature selection as performed in previous work, in which we manually identified and dropped features that were less important than randomly generated variables<a href='#References'><sup>[1]</sup></a>. 
...
# x11()
ggplot(auto_df %>%
         filter(type == "Confirmed" | type == "Tentative"), aes(y = features, x = importance, fill = type)) +
  geom_boxplot() + 
...
mean(ba[grep("INV", names(ba))]) # 98.75
std_err(ba[grep("INV", names(ba))]) # 0.0075
...
Overall, our native range validation results from our previous work were replicated here<a href='#References'><sup>[1]</sup></a>, and the patterns of overlap among introduced range individuals in acoustic space were similar to native range individuals.

Model validation yielded biologically relevant patterns, so I proceeded by using the SPCC and random forests similarity matrices for visualizations and Mantel tests. Since I extracted a proximity matrix for the individual scale validation calls and site scale calls together, all subsequent acoustic space visualizations made with the random forests proximity matrix can be interpreted along the same axes.
...
# References
    ...
    2. Keen, S., Ross, J. C., Griffiths, E. T., Lanzone, M., & Farnsworth, A. (2014). A comparison of similarity-based approaches in the classification of flight calls of four species of North American wood-warblers (Parulidae). Ecological Informatics, 21, 25–33. https://doi.org/10.1016/j.ecoinf.2014.01.001
