---
title: <center style="font-size:30px;font-style:normal;color:black;">Accompanying Code 06:</center>
subtitle: <center style="font-size:30px;font-style:normal;color:#0E0E7D;">Acoustic Space Figures</center>
 &nbsp;
author: |
...
```{r chunk1 load libraries echo = TRUE, eval = TRUE, message = FALSE}
...
Monk parakeet site scale datasets
...
Here, MDS was optimized to reduce stress, such that patterns in acoustic space reflected "optimal" solutions. Without optimization of MDS, it can be hard to tell whether patterns seen using the first two axes are biologically relevant, or due to a poor MDS solution in low dimensional space.

Iterate over as many dimensions possible given computation resources, perhaps 2 to 25. Save results, and plot stress for the individual scale, as well as the site scale across both similarity methods. Choose the number of dimensions that yields the lowest stress.
...
# Optimize MDS across social scales and similarity methods
...
Make a plot of RF acoustic space at the site scale between ranges
...
tmp_mds_df <- mds_df %>%
  ...
  # dplyr::mutate(
    ...
    # ids = factor(ids, levels = gsub("_2017|_2019", "", c(nat_sites, int_sites)))
  # ) %>% 
  inner_join(
    id_df,
    by = c("ids" = "site_ids")
...
tmp_mds_df <- mds_df %>%
  ...
  dplyr::filter(sim_meth == "SPCC") %>%
  dplyr::rename(`ids` = "Bird_ID") %>%
  filter(ids %in% c(t_indivs, v_indivs)) %>%
  dplyr::mutate(
    ids = as.character(ids),
...
nf_calls <- site_scale_nf %>%
  ...
  filter(site_year %in% c(nat_sites, int_sites)) %>%
  pull(sound.files)
...
site_datasets_mds_df <- rbindlist(pblapply(1:length(sim_meth), function(s){
  ...
  sub_df <- rbindlist(pblapply(1:length(datasets), function(d){
    ...
    tmp_df <- mds_df %>%
      filter(sim_meth == meth) %>%
      filter(sound.files %in% call_list[[d]]) %>%
      droplevels() %>%
      dplyr::mutate(
        dataset = datasets[d]
      )
