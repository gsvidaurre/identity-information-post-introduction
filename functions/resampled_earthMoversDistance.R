...
# Description: This function calculates Earth Mover's Distance (emdist::emd) for distributions of similarity values within or among categories (e.g. individuals or sites) at a given social scale. The function uses a histogram-based approach to create signatures or clusters compared between distributions (the analogy is to think of these signatures or clusters per distribution as piles of dirt). The function can iterate over different numbers of bins to calculate Earth Mover's Distance, or the minimum cost needed to turn one distribution into the other. The function also randomly samples an equivalent number of similarity values from each distribution, and this resampling can be performed over various iterations. This function was written for the datasets used in the accompanying manuscript, and will need to be updated before more generalized use with other datasets.

# Arguments:

# sim_df: A data frame representing pairwise comparisons within and among categories (e.g. individuals, sites) at the given social scale. This data frame should be the result of the customized function extract_simValues().
...
resampled_earthMoversDistance <- function(sim_df, distr_types = c("within", "among"), total_bins, bounds){
  ...
  res_df3 <- data.table::rbindlist(lapply(1:length(total_bins), function(b){
    ...
    # Create a vector of bins for the given number of total bins. From 0 to 1 or the absolute min and max possible in similarity space.
    ...
    # Account for the fact that among all of the original bins, some of these may not have been filled. This also needs to return the bins in order
    ...
    # From emdist::emd documentation: "emd uses first column of each matrix as the weigths and the remaining columns as location coordinates in a up to four-dimensional space. A and B must have the same number of columns."
    emd_res <- emdist::emd(
      A = mat_A, 
      B = mat_B,
      dist = "euclidean",
