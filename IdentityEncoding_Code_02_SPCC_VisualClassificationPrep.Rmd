---
title: "Identity Encoding Code 02:"
subtitle: "SPCC Similarity and Preparing for Visual Classification"
author: |
  <hr>
...
# List all known repeatedly sampled individuals per range
indivs <- list(
  nat_int_est %>%
    as_tibble() %>% # using tidyverse without conversion into a tibble leads to issues
...
SPCC_vals_df_tmp %>%
  ...
  geom_histogram(data = SPCC_vals_df_tmp %>%
                   dplyr::filter(type == "Same") %>% droplevels(),
                    aes(fill = type), bins = 100) +
  geom_histogram(data = SPCC_vals_df_tmp %>%
                   dplyr::filter(type == "Different") %>% droplevels(),
                    aes(fill = type), bins = 100) +
  facet_wrap(~ method) +
  scale_fill_manual(values = cols) +
...
SPCC on spectrograms picked up a cleaner split within versus among individuals than SPCC on cepstral coefficients. In other words, there was less overlap in the distributions of the within versus among individual similarity values using SPCC on spectrograms.
...
withn <- SPCC_vals_df %>%
  ...
  droplevels() %>%
  dplyr::filter(!is.na(SPCC_vals)) %>%
  pull(SPCC_vals)
head(withn)

amng <- SPCC_vals_df %>%
  dplyr::filter(method == "spectro") %>% 
  dplyr::filter(type == "Different") %>% 
  droplevels() %>%
  dplyr::filter(!is.na(SPCC_vals)) %>%
  pull(SPCC_vals)
head(amng)
...
Save the SPCC threshold for later use. The value below is what I used for thresholding which calls went into the Shiny app or not.
```{r eval = FALSE}
...
mBIC_res_list <- invisible(pblapply(1:length(site_year), function(x){
  ...
  icalls <- nat_int_est %>%
  dplyr::filter(social_scale == "Individual") %>%
  dplyr::filter(Bird_ID %in% indivs[[x]]) %>%
  dplyr::mutate(
  ...
  icalls <- unlist(lapply(1:length(indivs[[x]]), function(z){
    ...
    ic <- nat_int_est %>%
      dplyr::filter(social_scale == "Individual") %>%
      dplyr::filter(Bird_ID %in% indivs[[x]][z]) %>%
      dplyr::mutate(
        sound.files = as.character(sound.files) 
      ) %>%
      pull(sound.files)
...
nat_calls <- nat_int_est %>%
  dplyr::filter(social_scale == "Individual") %>%
  dplyr::filter(Bird_ID %in% paste("NAT", nat_indivs, sep = "-")) %>%
  pull(sound.files)
nat_calls
